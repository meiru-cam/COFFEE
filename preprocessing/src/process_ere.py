...
from transformers import (BertTokenizer, RobertaTokenizer,
                          XLMRobertaTokenizer, PreTrainedTokenizer, AutoTokenizer)
...
class Span:
    ...
    #             item in the list is a triple (start_offset, end_offset, text).
    #     """
    #     # Add this to handle cases like "&amp;"
    #     combined_tokens = []
    #     i = 0
    #     while i < len(tokens):
    #         if tokens[i][2] == '&amp;':
    #             combined_token = (tokens[i-1][0], tokens[i+1][1], ' '.join([tokens[i-1][2], tokens[i][2], tokens[i+1][2]]))
...
def tokenize(sentence: List[Tuple[int, int, str]],
             entities: List[Entity],
             events: List[Event],
             language: str = 'english'
             ) -> List[Tuple[int, int, str]]:
...
def process_batch(input_dir, output_file, dataset='normal', language='english'):
    ...
    annotation_files = glob.glob(os.path.join(input_dir, '*.xml'))

    with open(output_file, 'w', encoding='utf-8') as w:
        for annotation_file in annotation_files:
            doc_id = annotation_file.split("/")[-1][:-13]
            if dataset == 'discussion_forum':
                source_file = os.path.join('./Dataset/DEFT_English_Light_and_Rich_ERE_Annotation/data/source/discussion_forum', doc_id + ".cmp.txt")
            elif dataset == 'newswire':
                source_file = os.path.join('./Dataset/DEFT_English_Light_and_Rich_ERE_Annotation/data/source/newswire', doc_id + ".txt")
...
def main():
    ...
    datasets = ["discussion_forum", "newswire"]

    for dataset in datasets:
        json_path = os.path.join(args.output, '{}.{}.json'.format(args.lang,
                                                                dataset))
        process_batch(os.path.join(args.input, dataset),
                    json_path,
                    dataset=dataset,
                    language=args.lang
                    )
    ...
    if args.split:
        all_data = []
        for ds in ["discussion_forum", "newswire"]:
        ...
        with open(os.path.join(args.output, 'train.oneie.json'), 'w') as w_train, \
            open(os.path.join(args.output, 'dev.oneie.json'), 'w') as w_dev, \
            open(os.path.join(args.output, 'test.oneie.json'), 'w') as w_test:
