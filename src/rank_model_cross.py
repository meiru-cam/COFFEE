...
def trainer(model, all_batch_token, all_batch_mask, all_batch_seg, is_cuda, device, multigpu=False):
    ...
    for k in range(item_num):
        ...
        if is_cuda:
            if multigpu:
                batch_token_inp = batch_token_inp.cuda(1)
                batch_mask_inp = batch_mask_inp.cuda(1)
                batch_seg_inp = batch_seg_inp.cuda(1)
            else:
